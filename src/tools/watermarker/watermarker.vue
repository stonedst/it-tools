...
<template>
  <div>
    <c-file-upload :title="t('tools.watermarker.texts.title-drag-and-drop-images-here-or-click-to-select-some-files')"
      multiple accept="image/*" @files-upload="onFileChanges" />

    <NSelect v-if="images.length" v-model:value="selectedIndex" :options="imageOptions" label-field="label"
    ...
    <c-card :title="t('tools.watermarker.texts.title-settings')" mt-2>
      ...
      <NSpace justify="center">
        ...
        <n-form-item
          :label="repeatWatermark ? t('tools.watermarker.texts.label-tile-spacing') : t('tools.watermarker.texts.label-margin')"
          label-placement="left" mt-2>
        ...
        <n-form-item v-if="!repeatWatermark" :label="t('tools.watermarker.texts.label-position')" label-placement="left"
          mt-2>
          <NSelect v-model:value="watermarkPosition" style="min-width: 130px" :options="[
